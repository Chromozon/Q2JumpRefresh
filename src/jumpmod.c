...
"¡ÓÙÈÁÏıÂ ≈¡”Ÿ œŒÃŸ\n",
"This means antiglue is only available to those on EASY team. Type ·ÓÙÈÁÏıÂ to\n",
"switch on.\n\n",

"¡ÓÙÈÁÏıÂ œÊÊØœÓ\n",
"You may turn antiglue on and off at will on both teams. Type ·ÓÙÈÁÏıÂ to toggle.\n\n",
...
//		VectorCopy(targ->s.angles,spawn_angles);
		VectorClear (ent->velocity);
...
/*				added_spawn = false;
				for (i2=0;i2<MAX_ENTS;i2++)
				{
					...
					{
						if (strcmp(level_items.ents[i2]->classname,"info_player_deathmatch")==0)
						{
							added_spawn = true;
							break;
						}
...
void open_tourney_file(char *filename,qboolean apply)
{
	...
	while (!feof(f))
	{
		//problem here, its reading 1 too many? - LilRed: not anymore
		if (i>MAX_USERS)
			break;
...
void open_users_file()
{
	...
	fscanf(f, "%s", &name);
//	gi.dprintf("maplist version %s\n", va("%c%c%c", name[7], name[8], name[9]));
	if (Q_stricmp(va("%c%c%c", name[7], name[8], name[9]), "084") == 0)
	{
//		gi.dprintf ("JumpMod084 users file\n");
		maplist.version = 1;
	}
	else
	{
//		gi.dprintf ("JumpMod067 users file\n");
		maplist.version = 0;
	}
	//clear users file
	for (i=0;i<MAX_USERS;i++)
	{
//		maplist.users[i].inuse = false;
